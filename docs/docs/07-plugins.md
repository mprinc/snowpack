...
The **order** of plugin is important, as for example, if there are multiple plugins that load/build particular type of file, the first matching will take precedence.

To make the plugin available, you have to put it in your project `devDependency` list which will install it locally and make it available to snowpack.
